...
class _DashboardData {
  ...
  project; // per-project counts (null when none selected)
...
class _DashboardPageState extends State<DashboardPage> {
  ...
  Future<_DashboardData> _load() async {
    ...
    final selects = tables
        .map((t) => '(SELECT count(*) FROM $t) AS $t')
        .join(', ');
    final row = (await db.select(
      SqlStatement('SELECT $selects', const {}),
    )).rows.first;
    ...
    if (project != null) {
      ...
      final pr = (await db.select(
        SqlStatement(q, {'pid': project.id.value}),
      )).rows.first;
      projectCounts = {
        for (final k in [
          'modules',
    ...
    final metrics = (await injector.get<MetricsSummaryUsecase>()())
        .getOrDefault(const []);
      ...
      builder: (context, data) => ListView(
        ...
        children: [
          ...
          if (data.projectEntity != null) ...[
            _SectionLabel(
              l10n.t('dash.thisProject'),
              l10n.t('dash.thisProjectSub'),
            ),
            ...
            Wrap(
              ...
              children: [
                _MetricCard(
                  ...
                  l10n.t('dash.capModules'),
                ),
                _MetricCard(
                  l10n.t('dash.memories'),
                  data.project!['memories']!,
                  Icons.psychology_outlined,
                  l10n.t('dash.capMemories'),
                ),
                _MetricCard(
                  l10n.t('dash.rules'),
                  data.project!['rules']!,
                  Icons.rule_outlined,
                  l10n.t('dash.capRules'),
                ),
                _MetricCard(
                  l10n.t('dash.architectures'),
                  data.project!['architectures']!,
                  Icons.account_tree_outlined,
                  l10n.t('dash.capArch'),
                ),
                _MetricCard(
                  l10n.t('dash.sessions'),
                  data.project!['sessions']!,
                  Icons.forum_outlined,
                  l10n.t('dash.capSessions'),
                ),
                _MetricCard(
                  l10n.t('dash.requests'),
                  data.project!['requests']!,
                  Icons.question_answer_outlined,
                  l10n.t('dash.capRequests'),
                ),
                _MetricCard(
                  l10n.t('dash.handoffs'),
                  data.project!['handoffs']!,
                  Icons.swap_horiz_outlined,
                  l10n.t('dash.capHandoffs'),
                ),
                _MetricCard(
                  l10n.t('dash.tokens'),
                  data.project!['tokens']!,
                  Icons.toll_outlined,
                  l10n.t('dash.capTokens'),
                ),
              ],
            ),
          ...
          Wrap(
            ...
            children: [
              _MetricCard(
                l10n.t('dash.organizations'),
                data.global['organizations']!,
                Icons.inventory_2_outlined,
                null,
              ),
              _MetricCard(
                l10n.t('dash.projects'),
              ...
              _MetricCard(
                l10n.t('dash.handoffs'),
                data.global['handoffs']!,
                Icons.swap_horiz_outlined,
                null,
              ),
              _MetricCard(
                l10n.t('dash.searches'),
                data.global['agent_searches']!,
            ...
            Wrap(
              ...
              children: [
                if (project.createdAt != null)
                  MetaChip(
                    '${l10n.t('dash.created')}: ${fmtDateTime(project.createdAt)}',
                    icon: Icons.schedule,
                  ),
                MetaChip(
                  '${counts['memories']} ${l10n.t('dash.memories').toLowerCase()}',
                  icon: Icons.psychology_outlined,
                ),
                MetaChip(
                  '${counts['rules']} ${l10n.t('dash.rules').toLowerCase()}',
                  icon: Icons.rule_outlined,
                ),
                MetaChip(
                  '${counts['sessions']} ${l10n.t('dash.sessions').toLowerCase()}',
        ...
        child: Row(
          children: [
            Icon(
              ...
              size: 16,
              color: ok ? OracleBrand.success : OracleBrand.warning,
            ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                StatusBadge(
                  lint.clean
                      ? l10n.t('dash.healthy')
                      : l10n.t('dash.attention'),
                  color: lint.clean ? OracleBrand.success : OracleBrand.warning,
                ),
                const Spacer(),
                MetaChip(
                  '${l10n.t('dash.model')}: ${lint.currentModel}',
                  icon: Icons.memory,
                ),
              ],
            ),
            ...
            row(
              '${l10n.t('dash.memNoEmb')} — ${l10n.t('dash.memNoEmbSub')}',
              lint.memoriesWithoutEmbedding,
            ),
            const Divider(height: 1),
            row(
              '${l10n.t('dash.ruleNoEmb')} — ${l10n.t('dash.ruleNoEmbSub')}',
              lint.rulesWithoutEmbedding,
            ),
            const Divider(height: 1),
            row(
              '${l10n.t('dash.reqNoMsg')} — ${l10n.t('dash.reqNoMsgSub')}',
              lint.requestsWithoutMessages,
            ),
            const Divider(height: 1),
            row(
              '${l10n.t('dash.staleVec')} — ${l10n.t('dash.staleVecSub')}',
        ...
        child: SingleChildScrollView(
          ...
          child: DataTable(
            columns: [
              ...
              DataColumn(
                label: Text(l10n.t('dash.sessionsCol')),
                numeric: true,
              ),
              ...
              DataColumn(
                label: Text(l10n.t('dash.compactions')),
                numeric: true,
              ),
